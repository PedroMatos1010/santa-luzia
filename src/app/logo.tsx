...
export default async function Logo() {
    ...
    const res = await fetch('http://festa-santa-luzia-api.ddev.site/jsonapi/node/logo?page[limit]=1&include=field_imagem', {
        cache: 'no-store'
    });
    ...
    return (
        <Link href="/" className="relative block w-60 h-16 transition group hover:opacity-90">
            {urlImagem ? (
                // O Logótipo Dinâmico do Drupal
                <Image 
                    src={urlImagem} 
                    alt={logoNode.attributes.title} 
                    fill 
                    // object-contain garante que o logo cabe inteiro e não é cortado!
